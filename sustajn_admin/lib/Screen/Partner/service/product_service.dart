...
class OrderServices {
  ...
  Future<GetContainerData> getOrderService(String partUrl) async {
    try {
      ...
      if (response != null) {
        ...
        Utils.printLog("responseData in Service: ${responseData.message}");
        return responseData;
      } else {
  ...
  Future<ContainerHistoryData> getContainerByRestaurantService(String partUrl) async {
    try {
      Utils.printLog("requestData::::::: $partUrl");
      String url = NetworkUrls.BASE_URL + partUrl;
      ApiCallPresenter presenter = ApiCallPresenter();
  ...
  Future<LeaseBarrowData> getLeaseBorrowService(String partUrl) async {
    try {
      Utils.printLog("requestData::::::: $partUrl");
      String url = NetworkUrls.BASE_URL + partUrl;
      ApiCallPresenter presenter = ApiCallPresenter();
      var response = await presenter.getAPIData(url);
      if (response != null) {
        var responseData = LeaseBarrowData.fromJson(response);
        Utils.printLog("responseData in Service: ${responseData.message}");
        return responseData;
      } else {
        throw Exception(NetworkUrls.EMPTY_RESPONSE_CODE);
